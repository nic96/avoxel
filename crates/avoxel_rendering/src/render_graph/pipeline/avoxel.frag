...
layout(set = 3, binding = 0) uniform BlockMaterial_base_color {
    vec4 base_color;
};

#ifdef BLOCKMATERIAL_BASE_COLOR_TEXTURE
layout(set = 3, binding = 1) uniform texture2DArray BlockMaterial_base_color_texture;
layout(set = 3, binding = 2) uniform sampler BlockMaterial_base_color_texture_sampler;
#endif

#ifndef BLOCKMATERIAL_UNLIT

layout(set = 3, binding = 3) uniform BlockMaterial_roughness {
    float perceptual_roughness;
};

layout(set = 3, binding = 4) uniform BlockMaterial_metallic {
    float metallic;
};

#    ifdef BLOCKMATERIAL_METALLIC_ROUGHNESS_TEXTURE
layout(set = 3, binding = 5) uniform texture2DArray BlockMaterial_metallic_roughness_texture;
layout(set = 3, binding = 6) uniform sampler BlockMaterial_metallic_roughness_texture_sampler;
#    endif

layout(set = 3, binding = 7) uniform BlockMaterial_reflectance {
    float reflectance;
};

#    ifdef BLOCKMATERIAL_NORMAL_MAP
layout(set = 3, binding = 8) uniform texture2DArray BlockMaterial_normal_map;
layout(set = 3, binding = 9) uniform sampler BlockMaterial_normal_map_sampler;
#    endif

#    if defined(BLOCKMATERIAL_OCCLUSION_TEXTURE)
layout(set = 3, binding = 10) uniform texture2DArray BlockMaterial_occlusion_texture;
layout(set = 3, binding = 11) uniform sampler BlockMaterial_occlusion_texture_sampler;
#    endif

layout(set = 3, binding = 12) uniform BlockMaterial_emissive {
    vec4 emissive;
};

#    if defined(BLOCKMATERIAL_EMISSIVE_TEXTURE)
layout(set = 3, binding = 13) uniform texture2DArray BlockMaterial_emissive_texture;
layout(set = 3, binding = 14) uniform sampler BlockMaterial_emissive_texture_sampler;
#    endif
...
void main() {
    ...
    output_color *= texture(
        sampler2DArray(BlockMaterial_base_color_texture, BlockMaterial_base_color_texture_sampler),
        vec3(v_Uv, v_Layer));
#endif
...
#    ifdef BLOCKMATERIAL_METALLIC_ROUGHNESS_TEXTURE
    vec4 metallic_roughness = texture(sampler2DArray(BlockMaterial_metallic_roughness_texture, BlockMaterial_metallic_roughness_texture_sampler), vec3(v_Uv, v_Layer));
...
#    ifdef BLOCKMATERIAL_DOUBLE_SIDED
...
#    ifdef BLOCKMATERIAL_NORMAL_MAP
    mat3 TBN = mat3(T, B, N);
    N = TBN * normalize(texture(sampler2DArray(BlockMaterial_normal_map, BlockMaterial_normal_map_sampler), vec3(v_Uv, v_Layer)).rgb * 2.0 - 1.0);
#    endif

#    ifdef BLOCKMATERIAL_OCCLUSION_TEXTURE
    float occlusion = texture(sampler2DArray(BlockMaterial_occlusion_texture, BlockMaterial_occlusion_texture_sampler), vec3(v_Uv, v_Layer)).r;
#    else
    float occlusion = 1.0;
#    endif

#    ifdef BLOCKMATERIAL_EMISSIVE_TEXTURE
    vec4 emissive = emissive;
    // TODO use .a for exposure compensation in HDR
    emissive.rgb *= texture(sampler2DArray(BlockMaterial_emissive_texture, BlockMaterial_emissive_texture_sampler), vec3(v_Uv, v_Layer)).rgb;
#    endif
    ...
    output_color = mix(output_color, FogColor, v_FogAmount);
